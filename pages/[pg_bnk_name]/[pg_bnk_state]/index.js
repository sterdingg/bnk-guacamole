...
function index() {
    ...
    if (count == 1) {
        bank_db_filtered !== undefined && bank_db_filtered.map((item) => {
            if (item.STATE === routerdata.query.pg_bnk_state.replaceAll("_", " ")) {
                for_state_set.push(item.CITY2)
            }
        })
    ...
    // console.log(routerdata?.query)

    return (
        <>
        <HeadSeo headtype="Bank_State_Page" bank_name={routerdata?.query?.pg_bnk_name?.replaceAll("_", " ")} bank_state={routerdata?.query?.pg_bnk_state?.replaceAll("_", " ")}/>
            <form className="container pt-3">
                ...
                <p>State : {routerdata?.query?.pg_bnk_state!==undefined?routerdata?.query?.pg_bnk_state.replaceAll("_", " "):""}</p>
                    <div className="form-group col-md-12">
                        <label htmlFor="input_bankname">Select Bank City</label>
                        <div>
                            <select name="input_bankname" id="Bank Name" className="form-select" aria-label="Default select example" onChange={(e) => { handleChange(e, "bankcity") }}>
