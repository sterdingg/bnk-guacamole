...
export default function BranchV4({ details, nearby, bankName, bankUrl }) {
  ...
  if (router.isFallback) return <div className="container mt-5 text-center text-muted">Generating branch details, please wait...</div>;
  if (!details) return <div className="container mt-5 text-center text-danger">Branch not found.</div>;
  ...
  return (
    <>
      <HeadSeo />
      <Head>
        <title>{`${details.IFSC} - ${bankName} ${details.BRANCH} Branch IFSC Code, MICR Code | QPkendra`}</title>
        <meta name="description" content={`IFSC Code of ${bankName}, ${details.BRANCH} Branch in ${details.CITY1}, ${details.STATE} is ${details.IFSC}. Find address, contact, MICR code and more details.`} />
        <link rel="canonical" href={`https://bankifsccode.qpkendra.com/v4/${bankUrl}/${details.BRANCH.replaceAll(' ', '-')}`} />

        {/* Open Graph */}
        <meta property="og:type" content="website" />
        <meta property="og:title" content={`${details.IFSC} - ${bankName} ${details.BRANCH} IFSC Code`} />
        <meta property="og:description" content={`IFSC Code of ${bankName}, ${details.BRANCH} in ${details.CITY1}, ${details.STATE} is ${details.IFSC}. Get address, contact & MICR code.`} />
        <meta property="og:url" content={`https://bankifsccode.qpkendra.com/v4/${bankUrl}/${details.BRANCH.replaceAll(' ', '-')}`} />
        <meta property="og:site_name" content="Bank IFSC Code | QPkendra" />

        {/* Twitter */}
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" content={`${details.IFSC} - ${bankName} ${details.BRANCH} IFSC Code`} />
        <meta name="twitter:description" content={`IFSC Code of ${bankName}, ${details.BRANCH} in ${details.CITY1}, ${details.STATE} is ${details.IFSC}.`} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": bankName, "item": `https://bankifsccode.qpkendra.com/v4/${bankUrl}` },
            { "@type": "ListItem", "position": 4, "name": `${details.BRANCH} Branch` }
          ]
        })}} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "name": `${bankName} - ${details.BRANCH}`,
          "image": "https://bankifsccode.qpkendra.com/assets/favicon.png",
          "url": `https://bankifsccode.qpkendra.com/v4/${bankUrl}/${details.BRANCH.replaceAll(' ', '-')}`,
          "telephone": `+91 ${details.STD_CODE} ${details.PHONE}`,
          "address": {
            "@type": "PostalAddress",
            "streetAddress": details.ADDRESS,
            "addressLocality": details.CITY1,
            "addressRegion": details.STATE,
            "addressCountry": "IN"
          },
          "branchCode": details.BRANCH,
          "identifier": details.IFSC
        })}} />
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "mainEntity": [
            {
              "@type": "Question",
              "name": `What is the IFSC code of ${details.BRANCH} branch of ${bankName}?`,
              "acceptedAnswer": { "@type": "Answer", "text": `The IFSC code of ${details.BRANCH} branch of ${bankName} is ${details.IFSC}.` }
            },
            {
              "@type": "Question",
              "name": `What is the address of ${bankName} ${details.BRANCH} branch?`,
              "acceptedAnswer": { "@type": "Answer", "text": `The address of ${bankName} ${details.BRANCH} branch is ${details.ADDRESS}.` }
            },
            {
              "@type": "Question",
              "name": `What is the MICR code of ${bankName} ${details.BRANCH}?`,
              "acceptedAnswer": { "@type": "Answer", "text": `The MICR code of ${bankName} ${details.BRANCH} is ${details.MICR_CODE || 'N/A'}.` }
            },
            {
              "@type": "Question",
              "name": `What is the contact number of ${bankName} ${details.BRANCH}?`,
              "acceptedAnswer": { "@type": "Answer", "text": `The contact number is +91 ${details.STD_CODE} ${details.PHONE}.` }
            }
          ]
        })}} />
      </Head>
      ...
      <div className="container mt-4 mb-5 pb-5">
        ...
        <nav aria-label="breadcrumb">
          <ol className="breadcrumb" style={{ background: 'none', padding: 0 }}>
            ...
            <li className="breadcrumb-item active fw-bold" aria-current="page" style={{ color: '#4a5568', fontSize: '0.9rem' }}>{details.BRANCH} Branch</li>
          </ol>
        </nav>
            ...
            <div className="bg-white border rounded p-4" style={{ borderColor: '#e2e8f0' }}>
              ...
              {nearby.length > 0 && (
                <div className="mt-4 pt-2">
                  <a href="#" className="text-decoration-none fw-bold small" style={{ color: '#0056b3' }}>View all branches in {details.CITY1} &rarr;</a>
                </div>
              )}
            </div>
...
export async function getStaticProps({ params }) {
  ...
  const branchSearch = branchParam.toLowerCase();
  const branchDetails = bankData.find(item => item.BRANCH.replaceAll(" ", "-").toLowerCase() === branchSearch);

  if (!branchDetails) return { notFound: true };

  const nearby = bankData.filter(item => item.CITY1 === branchDetails.CITY1 && item.IFSC !== branchDetails.IFSC).slice(0, 4);
